...
To prevent reuse, uuid is a [ULID](https://github.com/ulid/spec), and its timestamp is confirmed to be valid upon creation. ULIDs are use-selected rather that server-generated so that they can be included in the POW and avoid replays while allowing a single POST to create a block.
